...
- リグニン吸着を低減するためにセルラーゼを改変すれば、セルラーゼの利用可能濃度を高めることができ、その結果、酵素の負荷量を低減し、リグノセルロース系バイオ燃料のコストを低減できる。
    ...
    - ほとんどの真菌セルラーゼは、触媒ドメイン（CD）と、柔軟で高度にグリコシル化されたリンカーで接続されたセルロース結合モジュール（CBM）からなる（Payne et al.、2015年）。
...
- セルラーゼのリグニンへの吸着に関する研究は数多くあるが、リグニンとの親和性を低下させるために酵素を設計する試みはほとんど行われていない。
    ...
    - リグニン親和性が低いセルラーゼ変異体(リグニン耐性セルラーゼ)を設計すれば、余分なコストや添加物の煩雑さなしで、リグニン阻害を低減できる。
- そこで、本研究では、部位特異的変異導入法を用いて、Cel7Aのリグニンに対するAffinityを調査・工学化した。
