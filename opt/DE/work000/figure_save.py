...
class figure_save_class:
    def get_contour_data(self, prob, delta):
        ...
        for i in range(0, mesh):
            for j in range(0, mesh):
                #x_ : (N, mesh, mesh)
                x_ = x[:, i, j]
                obj_box_[i, j] = get_obj_class().get_eval(x_, prob)
    ...
    def gbest_trend(self, figure_label, x, y1, y2, fig_file_name, scale='linear'):
        x_label_name = figure_label[0]
        ...
        if scale == 'linear':
            ...
            ax1.set_yscale(value=scale, nonpositive='mask')
        ...
        if scale == 'linear':
            ...
            #ax2.set_yscale(value=scale, nonpositive='mask')
            ax2.set_yscale(value=scale)
    ...
    def scatter_solution_space(self, figure_label, x1, x2, gbest, fig_file_name):
        ...
        # scatter
        ax.scatter(x1, x2, color="C0", s=40, alpha=1.0, marker="o", norm=Normalize(vmin=cmin, vmax=cmax),  label=label_name1)
    ...
    def scatter_trajectory_obj_vio_space_line(self, figure_label, obj_vio_box, w_box, fig_file_name):
        x_label_name = figure_label[0]
        y_label_name = figure_label[1]
        label_name1 = figure_label[2]
        label_name2 = figure_label[3]
        ...
        #obj_vio (iter_max, m, 2) => interval sample
        sample = 100
        m = w_box.shape[0]
        obj_vio_box = obj_vio_box[::sample, :, :]
        #obj_box = obj_vio_box[:, :, 0].reshape(-1)
        #vio_box = obj_vio_box[:, :, 1].reshape(-1)
        ...
        # xi scatter
        for i in range(0, m):
            ax.scatter(obj_vio_box[:, i, 0], obj_vio_box[:, i, 1], c='none', edgecolor=cmap(i), s=8, marker='o')
        ax.plot(obj_vio_box[:, 0, 0],obj_vio_box[:, 0, 1], linestyle='dotted', color=cmap(0), label=label_name1)
        ax.plot(obj_vio_box[:, m-1, 0],obj_vio_box[:, m-1, 1], linestyle='dotted', color=cmap(m-1), label=label_name2)
    ...
    def scatter_trajectory_obj_vio_space(self, figure_label, obj_vio_box, w_box, fig_file_name):
        x_label_name = figure_label[0]
        y_label_name = figure_label[1]
        label_name1 = figure_label[2]
        x_min = figure_label[3]
        x_max = figure_label[4]
        y_min = figure_label[5]
        y_max = figure_label[6]
