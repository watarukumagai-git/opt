...
- The idea which time-series data is denoised by combined with some decomposition or integration before prediction already exists. The exsiting studies [32-40] propose a framework that a prediction model is applied after time-series data is denoised by multiple signal decompositions. The additional decomposition to the part of frequency sequences after the first decomposition is a known technique as two-stage decomposition, secondary decomposition, and decomposition-reconstruction. Moreover, deriving the new integrated subsequences Co-IMFs from the decomposed components IMFs is used in some studies [37-40]. Especially, [37,38] apply the secondary decomposition to the high-frequency sequence after CEEMDAN and Sample Entropy. They are very similar to the denosing method in this paper.

On the other hand, I guess the originality and creativity of this paper are related to how to carry out denosing time-series data by combining with CEEMDAN, VMD, Sample Entropy, and clustering. However, I cannot judge the originality and creativity because it does not clearly state the proposal parts and the difference from other literature. 
