...
What are the originality and creativity of this paper? The originality and creativity written in the current manuscript may not reach the level required for publication in this journal for the following reasons:
   ...
   - The idea which time-series data is denoised by combined with some decomposition or integration before prediction already exists. The exsiting studies [32-35] propose a framework that a prediction model is applied after time-series data is denoised by multiple signal decompositions. The additional decomposition to the part of frequency sequences after the first decomposition is a known technique as two-stage decomposition / secondary decomposition [32-35]. Moreover, deriving the new integrated subsequences Co-IMFs from the decomposed components IMFs by Sample Entropy is used as some studies[35-36]. Especially, [34] apply VMD to the high-frequency sequence after CEEMDAN and k-means clustering using Sample Entropy. They are very similar to the denosing method in this paper.
