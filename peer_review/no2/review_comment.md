...
# [comments]
## [major comment1]
- 本論文の大きな目的は短期電力負荷予測の精度を改善だが、サブの目的は何か？いくつかのアルゴリズムを組み合わせて提案手法を構築した理由は、電力負荷データの時間的•空間的なゆらぎやランダム性を扱うためだと推察する。このため、Chapter1で、「本論文の目的は、電力負荷データの時間的•空間的なゆらぎやランダム性を扱うことで短期電力負荷予測精度の改善を図ることである」と明記すべきだ。

## [major comment2]
- 本論文の新規性•創造性はそれぞれ何か？
- 本論文のCEEMDAN-VMD、LSTM、DBOなどのアルゴリズムは、他の文献で開発されたものである。
- さらに、In my understanding, 対象の電力負荷予測も一般的によく知られている問題で、特に本論文で特別なシチュエーションを追加していないと推察する。
- 
- 様々なタスクに適応するために、機械学習や深層学習のハイパーパラメータを上位の最適化機能によって自動調整するアイディアは新しくない。
- 例えば、文献[28]では、最適化機能との併用によって、電力負荷予測ではないが、様々なタスクにおいて予測性能を改善させている。よって、最適化機能の導入は、近年の深層学習において特別なアイディアではなく、深層学習を実用的な性能を引き出す上で必要な作業とも言い換えられる。
- また、最適化アルゴリズムとして電力負荷予測問題にDBOを採用した理由は何か？もしこの答えが「4.5節の実験を通して最も優れていたため」ならば、それは本論文のアイディアとは言えない。つまり、
